...
// Get cosplay slug or ID from query string
$slug = $_GET['slug'] ?? '';
$id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

if (empty($slug) && $id <= 0) {
    header('Location: cosplay.php');
    exit;
}

// Fetch cosplay from database
$cosplay = null;
$cosplay_images = [];
...
try {
    ...
    // Fetch cosplay images
    $imgStmt = $pdo->prepare("SELECT id, image_path, alt_text, display_order, grid_size FROM cosplay_images WHERE cosplay_id = ? ORDER BY display_order ASC, id ASC");
    $imgStmt->execute([$cosplay['id']]);
    $cosplay_images = $imgStmt->fetchAll();
    
} catch (Exception $e) {
    error_log("Failed to load cosplay: " . $e->getMessage());
    header('Location: cosplay.php');
    exit;
}

$page_title = htmlspecialchars($cosplay['title'] ?? 'Cosplay');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <meta name="description" content="<?php echo htmlspecialchars($cosplay['description'] ?? ''); ?>">
...
<body>
    ...
    <section id="cosplay" class="project-section">
        <div class="project-detail">
            <div class="project-header">
                <a href="cosplay.php" class="back-link">
                    <i class="fas fa-arrow-left"></i>
                    <span>Back to Cosplay</span>
                </a>
                <h1><?php echo htmlspecialchars($cosplay['title']); ?></h1>
                <?php if (!empty($cosplay['image_path'])): ?>
                <div class="project-thumbnail">
                    <img src="<?php echo htmlspecialchars($cosplay['image_path']); ?>" 
                         alt="<?php echo htmlspecialchars($cosplay['alt_text'] ?? $cosplay['title']); ?>" />
                </div>
            <?php endif; ?>

                <?php if (!empty($cosplay['description'])): ?>
                    <div class="project-description">
                        <?php echo nl2br(htmlspecialchars($cosplay['description'])); ?>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <?php if (!empty($cosplay_images)): ?>
                <div class="project-images-grid" id="cosplay-images-grid">
                    <?php foreach ($cosplay_images as $img): ?>
    ...
    <script>
        ...
        if (thumbnailImg) {
            images.push({
                src: '<?php echo htmlspecialchars($cosplay['image_path']); ?>',
                alt: '<?php echo htmlspecialchars($cosplay['alt_text'] ?? $cosplay['title']); ?>'
            });
