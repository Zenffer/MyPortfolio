...
$uploadDir = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'img' . DIRECTORY_SEPARATOR . 'content' . DIRECTORY_SEPARATOR . 'cosplay';
if (!is_dir($uploadDir)) {
...
$ext = $allowed[$mime];
$basename = 'cosplay_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
...
$publicUrl = 'assets/img/content/cosplay/' . $basename;

// Return success with the new URL
echo json_encode([
    'ok' => true, 
    'url' => $publicUrl,
    'message' => 'Cosplay image uploaded successfully'
]);
?>
