...
try {
    ...
    $uploadDir = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'img' . DIRECTORY_SEPARATOR . 'content' . DIRECTORY_SEPARATOR . 'projects' . DIRECTORY_SEPARATOR;
    ...
    $maxOrder = (int)$pdo->query("SELECT COALESCE(MAX(display_order),0) FROM project_images WHERE project_id = " . $project_id)->fetchColumn();
    ...
    for ($i = 0; $i < $fileCount; $i++) {
        ...
        $filename = 'project_' . $project_id . '_' . time() . '_' . $i . '.' . $extension;
        $filepath = $uploadDir . $filename;
        
        if (move_uploaded_file($files['tmp_name'][$i], $filepath)) {
            $relativePath = 'assets/img/content/projects/' . $filename;
            
            // Insert into database
            $stmt = $pdo->prepare("INSERT INTO project_images (project_id, image_path, display_order, grid_size) VALUES (?, ?, ?, ?)");
            $stmt->execute([$project_id, $relativePath, $currentOrder, $grid_size]);
