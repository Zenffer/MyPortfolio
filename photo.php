...
try {
    ...
    $imgStmt = $pdo->prepare("SELECT id, image_path, alt_text, display_order, grid_size FROM photography_images WHERE photography_id = ? ORDER BY display_order ASC, id ASC");
    $imgStmt->execute([$photo['id']]);
    $photo_images = $imgStmt->fetchAll();
    
} catch (Exception $e) {
    error_log("Failed to load photo: " . $e->getMessage());
    header('Location: photography.php');
    exit;
}

$page_title = htmlspecialchars($photo['title'] ?? 'Photo');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <meta name="description" content="<?php echo htmlspecialchars($photo['description'] ?? ''); ?>">
...
<body>
    ...
    <section id="photo" class="project-section">
        <div class="project-detail">
            <div class="project-header">
                <a href="photography.php" class="back-link">
            ...
            <?php if (!empty($photo_images)): ?>
                <div class="project-images-grid" id="photo-images-grid">
                    <?php foreach ($photo_images as $img): ?>
    ...
    <script>
        ...
        if (thumbnailImg) {
            images.push({
                src: '<?php echo htmlspecialchars($photo['image_path']); ?>',
                alt: '<?php echo htmlspecialchars($photo['alt_text'] ?? $photo['title']); ?>'
            });
